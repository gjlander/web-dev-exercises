...
const App = () => {
  ...
  const handleChange = e => {
    e.target.type === 'file' && setImage(URL.createObjectURL(e.target.files[0]));
    setForm(prev =>
      e.target.type === 'file'
    ...
    <div className='container mx-auto'>
      ...
      <form className='mt-5 w-1/3 mx-auto flex flex-col items-center gap-5' onSubmit={handleSubmit}>
        <label className='input input-bordered flex items-center gap-2 w-full'>
          Name:
          <input value={form.name} onChange={handleChange} type='text' name='name' className='grow' />
        </label>
        <label className='input input-bordered flex items-center gap-2 w-full'>
          Quote:
          <input value={form.quote} onChange={handleChange} type='text' name='quote' className='grow' />
        </label>
