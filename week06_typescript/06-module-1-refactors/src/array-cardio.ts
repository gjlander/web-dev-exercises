...
const mathematicians = inventors.filter(
  inventor => inventor.categories && inventor.categories.includes('mathematician')
);
...
const malePhysicists = inventors.filter(
  inv => inv.categories?.includes('physicist') && inv.categories?.includes('man')
);
...
//concatenation
const inventorEmails = inventors.map(inv => inv.first.toLowerCase() + inv.year + '@inventor.com');
...
const totalYearsLived = inventors.reduce(
  (accumulator, currentInventor) => accumulator + (currentInventor.passed - currentInventor.year),
  0
);
...
const boulevardsWithDe = boulevardsInParis.filter(boul => boul.includes('de'));
console.log('boulevardsWithDe: ', boulevardsWithDe);

const boulevardsWithDeStrict = boulevardsInParis.filter(boul => boul.includes(' de '));
