const checkInCart = (cart, product) => cart?.find(item => product.id === item.id);

const calcCartCost = cart => cart.reduce((acc, item) => acc + item.price * item.count, 0);
...
const addToCart = (cart, product) => {
  ...
  if (!prodInCart) {
    ...
    updatedCart = cart.map(item => (item.id === product.id ? { ...item, count: item.count + 1 } : item));
  }
...
export { addToCart, calcCartCost, calcCartCount, checkInCart, removeFromCart, formatPrice };
